...
@near @interrupt void ADC2_IRQHandler (void)
{
    ...
    else if ((Conversion_Value <= 0xFF) && (Conversion_Value > 0x0))
    {
        ...
        GPIO_WriteLow(LEDS_PORT, LED3_PIN|LED2_PIN|LED4_PIN);
    }
    else if ((Conversion_Value >= 0x100) && (Conversion_Value <= 0x1FF))
    {
        GPIO_WriteHigh(LEDS_PORT, LED2_PIN|LED1_PIN);
        GPIO_WriteLow(LEDS_PORT, LED3_PIN|LED4_PIN);

    }
    else if ((Conversion_Value >= 0x200) && (Conversion_Value <= 0x2FF))
    {
        GPIO_WriteHigh(LEDS_PORT, LED3_PIN|LED2_PIN|LED1_PIN);
        GPIO_WriteLow(LEDS_PORT, LED4_PIN);
